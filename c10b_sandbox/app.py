"""
C10B_SANDBOX — Lean coding/test sandbox (FastAPI, port 8310).

Cloned from C12b with different port (8310 vs 8210) to avoid conflicts.
...
SESSION_DB = Path(os.environ.get("SESSION_DB_PATH", str(Path.home() / ".c10b_sessions.db"))).resolve()

app = FastAPI(title="C10b Lean Sandbox", version="1.0.0")
...
async def exec_command(req: ExecRequest):
    ...
    session_id = (req.session_id or "").strip() or ("c10b_" + uuid.uuid4().hex[:10])
...
if __name__ == "__main__":
    ...
    uvicorn.run("app:app", host="0.0.0.0", port=8310, reload=False)
