...
```bash
# Verify all key services are up
...
If your machine has limited RAM, start CORE only and bring up groups on demand:
...
> **Tip:** The **Container Manager** at `http://localhost:6090/` lets you start/stop/restart/rebuild any group visually with one click — no CLI needed.

### 8.2 Full clone → build → auth → run (reference)
